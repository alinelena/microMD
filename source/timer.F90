...
Contains
  ...
  Subroutine init_timer_system ( tmr, nrite,stack_depth )
    !!------------------------------------------------!
    !!
    ...
    if  (present(stack_depth)) then
         tmr%max_depth = stack_depth
    end if
  ...
  Subroutine timer_report(tmr)
    ...
    Type( timer_type ), Intent( InOut ) :: tmr
    Type ( node ), Pointer :: current_timer
    Character( Len = 250 ), Dimension(:), Allocatable :: message
    Integer :: nprocs
    Integer :: proc
    Integer :: ierr
  ...
  Subroutine timer_print_tree(tmr, init_node, max_depth, proc_id, message)
    ...
    Character(Len = 250 ) :: fline,fhead,fcontent
    ...
    Write (message(-2), fmt=Trim(fline))
    Write (message(-1), fmt=Trim(fhead))
...
!         & "|",1X,F9.4,1X,"|",1X,F9.4,1X,"|",1X,F9.4,1X,"|",1X,F9.4,1X,"|",2X,F8.4,1X,"|")
  ...
  Subroutine timer_last_time(tmr, name, screen)
    ...
    If (to_screen) Then
       Write(0,'(a)') current_timer%time%name, current_timer%time%calls, current_timer%time%last
    Else
  ...
  Subroutine timer_write_sing(message, timer_in)
    ...
    If (Present(timer_in)) Then
       ...
       If ( timer_in%proc_id == 0 ) Then
          Write(timer_in%out_unit,'(a)') message
       End If
